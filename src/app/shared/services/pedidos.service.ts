...
pedidoPendienteSubject = new BehaviorSubject<Pedido[]>([]);
  ...
  constructor(private firestore: Firestore) { }
    ...
    // SERVICIO PARA TRAER TODOS LOS PEDIDOS EN UN ARRAY
    getPedidos(tabla:string): Promise<void> {
      const clientesRef = collection(this.firestore, tabla);
      return getDocs(clientesRef).then(snapshot => {
        const pedidos: Pedido[] = snapshot.docs.map(doc => ({
          ...doc.data() as Pedido,
          id: doc.id
        }));
        if(tabla==='Pedidos Pendientes'){
          this.pedidoPendienteSubject.next(pedidos);
        }
        else{
          this.pedidoFinalizadoSubject.next(pedidos);
        }
